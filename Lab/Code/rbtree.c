...
{
    if (!root)
        return; // *root == NULL means empty tree
...
void RBDestroy(RBNode **root, void (*destroy)(void *))
{
    if (!root || !*root)
        return;
    if ((*root)->left)
        RBDestroy(&((*root)->left), destroy);
    if ((*root)->right)
        RBDestroy(&((*root)->right), destroy);
    if (destroy != NULL)
        destroy((*root)->value);
    free(*root);
    *root = NULL;
...
void RBTranverse(RBNode **root, void (*handle)(void *)) {
    if (!root || !*root) return;
    if (handle != NULL) handle((*root)->value);
    if ((*root)->left) RBTranverse(&((*root)->left), handle);
    if ((*root)->right) RBTranverse(&((*root)->right), handle);
}